...
- **`ts:build` and `test` cannot pass until `pre-build` has run at least once** — against the committed stubs, `packages/cli` reports ~200 `TS2305: has no exported member` and `packages/playground` cascades into `TS6305`, which reads like a broken branch rather than a missing generate step. Same for `bun run test`: the CLI process tests spawn the CLI and it dies with `SyntaxError: Export named … not found`. Run `bun run pre-build` first, then restore the stubs as above.
