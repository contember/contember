...
| `options.mailVariant` | Override the mail template variant — see [mail templates](./mail-templates.md). |
...
The password is validated against the active [password policy](./password-policy.md) including the optional HIBP check.
...
- Records `password_reset` in the [audit log](./audit-log.md).
