...
A successful email change records `email_change` in the [audit log](./audit-log.md).
...
| `NO_PASSWORD_SET` | The person has no password (IDP-only / passwordless-only). Use [password reset](./password-reset.md) to *set* the first password. |
| `INVALID_PASSWORD` | `currentPassword` is wrong. |
| `TOO_WEAK` | `newPassword` failed [password policy](./password-policy.md). `weakPasswordReasons[]` carries reasons including `COMPROMISED` *(since 2.2)*. |
...
- Setting a temporary password before invoking [`forceSignOutPerson`](./sessions.md#admin-force-sign-out) to lock the person out of their existing sessions.
- Bulk migration scripts.

The new password is validated against the active [password policy](./password-policy.md) just like the self-service path. Records `password_change` in the audit log.
...
- Trigger the public [password reset](./password-reset.md) flow.
