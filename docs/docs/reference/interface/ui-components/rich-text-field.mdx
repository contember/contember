...
### Playground – Alignment, Link, and Mark Controls
```tsx
<Binding>
	<Slots.Title>
...
```
Shows a fully-featured inline toolbar with alignment buttons and a link trigger.

### Lesson Description – Learning Platform
```tsx
<FormLayout className="mx-0">
	<InputField field="name" />
	<RichTextField
		field="description"
		label={entityLabels.CourseLessonLocale.fields.description}
	/>
</FormLayout>
```
A simple form section binding the `description` field of a lesson locale.

### Conditional Minimal Variant
```tsx
<FormLayout>
	{variant === 'minimal' ? (
		<>
			...
			<RichTextField
				field="description"
				label={entityLabels.CourseModuleLocale.fields.description}
			/>
		</>
	) : null}
</FormLayout>
```
Shows how you can render the editor only when a certain variant is active.

### Notification Email Body
```tsx
<FormLayout>
	<EmailInputField field="emailSubject" />
	<RichTextField
		field="emailBodyPart"
		label={entityLabels.NotificationLocale.fields.emailBodyPart}
	/>
</FormLayout>
```
Uses RichTextField for composing notification emails.

### Global Email Body Part with Locales
```tsx
<FormLocales>
	<FormLayout>
		<RichTextField
			field="globalEmailBodyPart"
			label={entityLabels.NotificationRulesetLocale.fields.globalEmailBodyPart}
			description={
				entityLabels.NotificationRulesetLocale.form.description.globalEmailBodyPart
			}
		/>
	</FormLayout>
</FormLocales>
```
Demonstrates locale-aware editing of a global email template.
