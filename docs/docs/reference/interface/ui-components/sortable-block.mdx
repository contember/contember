---
title: SortableBlock
sidebar_label: SortableBlock
---

# SortableBlock

## Overview
SortableBlock wraps Slate editor blocks and makes them draggable.  
You add it to a block plugin to let users reorder content with a mouse or touch.

The component gives instant visual feedback and draws an overlay while you drag.  
It never mutates Slate state by itself; it only forwards events to the `withSortable` plugin.
...
* The accompanying `withSortable` plugin updates Slate when the drag ends.
...
```
You pass a `canDrop` function to the plugin, not to `SortableBlock`.  
The block now drags only inside its original section.
