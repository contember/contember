...
* The hook maps technical persistence errors to user-friendly toast notifications.
...
### Basic usage inside any async function
```tsx
const handleError = usePersistErrorHandler()
...
```
You call `handleError` in the `catch` block. The user immediately sees a toast describing the problem.
