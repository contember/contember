...
### Basic usage with a promise chain
```tsx
const handleSuccess = usePersistSuccessHandler()

persistData().then(result => handleSuccess(result))
```
Call the handler in the `then` callback to show a toast once the request resolves.
...
```
`handleSuccess` shows a toast (for example “Profile updated”) when the API responds successfully.
