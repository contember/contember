...
> We are using high level ACL definition API. For some cases, you might prefer a [low level definition API](/reference/engine/schema/acl#low-level-definition)

### Public role definition

First, we create a public role using [createRole](/reference/engine/schema/acl#create-role) function.

There is only single mandatory argument - a role identifier. In second argument, we can define various role options, as described [here](/reference/engine/schema/acl#create-role).
...
Second, we assign an access rule to a `Comment` entity using [allow](/reference/engine/schema/acl#allow) function.
...
You can manage this variable [on memberships using Tenant API](/reference/engine/tenant/memberships) using its name - `categoryId`.
...
:::note[migrations]
Don't forget to [create a migration](/reference/engine/migrations/basics) to apply changes:
```bash
