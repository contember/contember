...
Applies a declarative `tenant.config.ts` (global config, identity providers, mail templates) to a tenant, idempotently. See [declarative tenant configuration](/reference/engine/tenant/declarative-config).
