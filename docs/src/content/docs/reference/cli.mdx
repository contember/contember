...
See [migrations chapter](/reference/engine/migrations/overview).

### tenant:apply

Applies a declarative `tenant.config.ts` (global config, identity providers, mail templates, custom roles) to a tenant, idempotently. See [declarative tenant configuration](/reference/engine/tenant/declarative-config).
