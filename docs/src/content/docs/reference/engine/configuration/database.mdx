...
This works with a single replica or a reader endpoint that distributes connections across several replicas, including an Aurora reader endpoint. It uses the configured primary connection directly and does not depend on replication-status functions or replica identities.

The header is disabled by default, because any authenticated caller could use it to move its reads to the primary. Enable it for all projects:
...
While it is disabled, the engine ignores the header and does not mark mutation responses, so clients never open a primary read window. The header accepts the same values as `X-Contember-Force-Ok`: `1`, `true`, `on` or `yes`.
...
Content clients within the provider share a window when they use the same Content API URL and session token, including clients under a nested `ApiBaseUrlContext` or `SessionTokenContext` provider. Changing the window duration resets the windows. Tenant and System API clients do not use them.
