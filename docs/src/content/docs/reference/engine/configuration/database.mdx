...
`ContemberClient` automatically forces primary reads for **1000 ms after receiving a Content API mutation response**. Another mutation response restarts the window. Queries do not extend it. Configure the duration with `primaryReadWindowMs`, or set it to `0` to disable the window:
