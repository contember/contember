...
Content clients within the provider share a window for the same project and stage. Changing the session token, API base URL, or window duration resets the windows. Tenant and System API clients do not use them.
...
The `primaryReadWindow` option is also accepted per request. Passing `false` disables both sending the window's header and updating the window for that request. Explicit request headers take precedence over the window's header. `withOptions()` retains the same window unless you replace or disable it.

The engine marks Content API mutation responses with **`X-Contember-Mutation: 1`**. The client reads this header before processing the response body or invoking response callbacks, so a mutation that partially committed and then returned GraphQL errors still opens the window. The marker identifies an executed mutation operation, not a confirmed database write: write-free or failed mutations can also open a window. Responses without the marker do not open one, including responses from older engines. A lost mutation response cannot open a window.
