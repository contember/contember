...
A **query** that exceeds `maxBytes` fails as a whole with HTTP `422`:
...
Once a request exceeds its budget, its remaining mutations are not started and report the same error. Retrying the same request does not help; select less data (add `limit`, narrow the selection) or wrap dependent mutations in a `transaction` so that they succeed or fail together.
