...
The GraphQL schema provided by the Content API is restricted based on the [ACL rules](/reference/engine/schema/acl) defined in your project. This means that the API will only include the types, fields, and mutations that are allowed based on the permissions granted to the user associated with the access token. For example, if you have defined a read-only role for a mobile application, the GraphQL API will not include any mutations for this role.

Now you can discover how to read your data using [GraphQL queries](/reference/engine/content/queries) or modify them using [GraphQL mutations](/reference/engine/content/mutations). Subscriptions for watching data changes are currently not available.
