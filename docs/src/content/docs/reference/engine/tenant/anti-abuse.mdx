...
The Tenant API has five layered defenses against credential stuffing, account enumeration, password spraying, and resource abuse. They are independent — each can be enabled and tuned separately — and all live behind the [`configure`](/reference/engine/tenant/configuration) mutation.
...
| Layer | `signUp` | `signIn` | `createResetPasswordRequest` | `initSignInPasswordless` | `requestEmailVerification` |
|---|---|---|---|---|---|
| Captcha | ✅ | — | ✅ | ✅ | ⚙️ opt-in |
| Per-IP rate limit | ✅ `signUpPerIp` | ✅ `loginPerIp` | ✅ `passwordResetPerIp` | ✅ `passwordlessInitPerIp` | ✅ `emailVerificationPerIp` |
| Per-email backoff | — | ✅ (login backoff) | ✅ (mail-init backoff) | ✅ (mail-init backoff) | ✅ (mail-init backoff) |
...
### Which flows enforce captcha

The provider/secret is shared, but enforcement is **per flow** via `captcha.protect` — so you protect exactly the mutations you expose publicly. `signUp`, `passwordReset` (`createResetPasswordRequest`), and `passwordlessInit` (`initSignInPasswordless`) default to **on**; `emailVerification` (`requestEmailVerification`) defaults to **off** — opt in only if you expose that resend endpoint to anonymous clients.

```graphql
mutation {
  configure(config: {
    captcha: {
      provider: turnstile, secret: "0x4AAA…"
      protect: { signUp: true, passwordReset: true, passwordlessInit: true, emailVerification: false }
    }
  }) { ok }
}
```

`signIn` never takes a captcha — it neither creates a resource nor sends mail, so credential abuse is handled by the per-email login backoff and `loginPerIp` instead. The flags have no effect while `provider` is `null`.
...
- In the [audit log](/reference/engine/tenant/audit-log), `configure` calls record the rest of the input but redact `captcha.secret` to `***` — so you can see *when* the secret rotated without preserving the value.
...
The socket IP, unless the request is authenticated with an api_key that has `trustForwardedClientInfo` set and the proxy is sending `X-Contember-Client-IP`. See [proxy trust](/reference/engine/tenant/proxy-trust).
...
See [configuration → recommended hardening baseline](/reference/engine/tenant/configuration#recommended-hardening-baseline) for a copy-paste mutation.
