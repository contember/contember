...
| `unusual_login_detected` | *(since 2.3)* A password `signIn` scored as unusual by [anomaly detection](/reference/engine/tenant/anomaly-detection) — written when an `UNUSUAL_LOGIN` email is sent and on the step-up (`OTP_REQUIRED`) leg |
| `step_up_required` | *(since 2.3)* Anomaly detection forced a second factor before issuing the session (written alongside `unusual_login_detected` on the step-up leg) |
...
- `unusual_login_detected` and `step_up_required` *(since 2.3)* carry `event_data = { score, reasons }` — the cumulative risk score and which signals fired (e.g. `["new_country", "new_device"]`).
