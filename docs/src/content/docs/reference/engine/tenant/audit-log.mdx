...
| `idp_role_mapping_failed` *(since 2.2)* | `signInIDP` / session refresh — a configured membership mapping failed and was skipped (fail-open) | *(usually no event data — no claim values are ever stored)* — sign-in/refresh still succeeded; the mapping was not applied. When the failed mapping also configures a [membership lease](/reference/engine/tenant/idp#membership-lease), `{reason: "mapping_invalid_lease_unrenewed"}` records that nothing renewed it either, so its grants will lapse |
| `idp_membership_lease_expired` *(since 2.2)* | The housekeeping sweep removed claim-granted memberships whose [membership lease](/reference/engine/tenant/idp#membership-lease) was never renewed | `{memberships: [{project, role, identityProviderId}]}` — one entry per person, listing everything they lost in that sweep; `project` is the project slug and `identityProviderId` the provider whose mapping had granted and renewed that membership |
...
`idp_membership_lease_expired` is the one entry here with **no actor**: nobody acted — the lease simply ran out — so `invoked_by_id` is null, and so is `identity_provider_id`, because the sweep is global and the sign-in that happened to trigger it is not the provider whose grant lapsed (the granting provider is per membership, inside `event_data`). Both `person_id` and `target_person_id` are the person who lost the memberships.
