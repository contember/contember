...
| `anomalyDetection` | *(disabled)* | *(since 2.3)* Sign-in [anomaly detection](/reference/engine/tenant/anomaly-detection) policy: `{ enabled, historySize, emailThreshold, stepUpThreshold }`. Opt-in; off by default. |
