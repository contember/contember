...
| `authPolicies` | `createAuthPolicy` / `updateAuthPolicy` | `auth_policy` rows have no slug, so an entry is matched to an existing policy by what it targets: `scope` + `project` + the `roles` **set** (order is irrelevant). No match means a new policy. |
