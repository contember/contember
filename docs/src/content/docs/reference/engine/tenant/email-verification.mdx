...
When `emailChange.requireVerification` is **on** (it is **off** by default), a user changing their own address via [`changeMyProfile`](/reference/engine/tenant/profile) does **not** swap the address immediately. Instead:
...
When the flag is **off** (the default), `changeMyProfile` swaps the address immediately as before, so an engine upgrade does not change existing behavior. The admin [`changeProfile`](/reference/engine/tenant/profile#admin) mutation is **never** routed through verification — admins are trusted by policy and the change is applied directly.
...
| `emailChange.requireVerification` | `false` | Self-service e-mail changes go through `confirmEmailChange` instead of swapping immediately. |
