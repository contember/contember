...
Sign-in attempts via IdP are recorded as `idp_login`. When [session re-validation](#session-re-validation) revokes a session, it is recorded as `idp_session_revoked` with `event_data: {reason}` (e.g. `invalid_grant`, `idp_session_expired`, `idp_disabled`).
