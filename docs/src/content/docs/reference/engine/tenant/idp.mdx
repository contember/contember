...
- **Soft** — once the token passes `softRefreshThreshold` of its lifetime (default `0.5`, i.e. half-expired), a **background** refresh is triggered. It runs *after* the response, so the request keeps zero added latency, and the token is renewed before it dies. Revocation discovered here applies to the next request.
...
| `userinfo` | Calls the userinfo endpoint with the stored access token | No refresh token — the session lives only as long as the access token. |
| `introspection` | RFC 7662 token introspection | Session ends when the token is reported inactive. |
