...
| `exclusive` | `false` | Never link to an existing account by e-mail, and create auto-signed-up accounts **without** an e-mail address, so they hold no local credential at all. To keep the address and still refuse local sign-in, use `disableLocalAuthentication`. |
...
| `disableLocalAuthentication` | `false` | *(since 2.2)* A person linked to this provider may sign in **only** through it: password and passwordless sign-in answer `IDP_REQUIRED`, no password-reset mail is sent, and the person cannot disconnect the link. Disabling the provider lifts the restriction. See [IdP-only authentication](#idp-only-authentication). |
...
### IdP-only authentication

:::note[Available since 2.2]
:::

[Claim mapping](#claim-mapping) can make a provider authoritative for a person's memberships, but it only runs when the person signs in *through the provider*. A password, a magic link or a password-reset mail would still let someone the directory has dropped sign in with the memberships they last held. `options.disableLocalAuthentication` closes those doors: a person linked to such a provider may authenticate **only** through it.

| Path | Answer |
|---|---|
| `signIn` | `IDP_REQUIRED`, before the password is compared |
| `initSignInPasswordless`, `signInPasswordless` | `IDP_REQUIRED` — a magic link issued before the person was linked is refused too |
| `createResetPasswordRequest` | `ok: true`, no mail sent |
| `disconnectMyIdentityProvider` | `IDP_REQUIRED` — the link is what enforces the policy, so the person cannot remove it |

The marker is the person's [connection](#personal-idp-connections) to the provider, so it also covers an account that predates the provider and was linked by e-mail matching, not only one created by `autoSignUp`. Sessions that already exist stay valid; the option only changes how new ones are obtained. Permanent API keys and `createSessionToken` are unaffected.

`IDP_REQUIRED` names the provider, which reveals both that the person exists and how they sign in, so it is masked like `NO_PASSWORD_SET`: unless both `login.revealUserExists` and `login.revealLoginMethod` are `true`, `signIn` answers `INVALID_CREDENTIALS` and `initSignInPasswordless` answers `PASSWORDLESS_DISABLED`.

**Break-glass.** A [disabled provider](#temporarily-disabling-and-enabling-an-idp) does not count: `disableIDP` hands local sign-in back to everyone linked to it, with no deploy, and `enableIDP` takes it away again. The caller of `disableIDP` must still be able to authenticate — keep a permanent API key, or an administrator who is not linked to the provider, for the case where the provider itself is the outage. Disconnecting the link stays refused while the provider is disabled; to let people remove it, turn the option off with `updateIDP`.
...
*(since 2.2)* A connection to a provider with [`disableLocalAuthentication`](#idp-only-authentication) cannot be removed by the person at all, not even while the provider is disabled — the link is what binds them to the provider, so removing it would lift the policy.
...
| `IDP_REQUIRED` | *(since 2.2)* The provider has `disableLocalAuthentication`, so this connection is the only way the person may sign in. An operator can turn the option off with `updateIDP`. |
...
When [session re-validation](#session-re-validation) revokes a session, it is recorded as `idp_session_revoked` with `event_data: {reason, apiKeyId}` (`reason` is e.g. `invalid_grant`, `inactive`, `idp_disabled`); a token rotation is recorded as `idp_session_revalidated` with `event_data: {apiKeyId}`. `apiKeyId` identifies the affected session (the `id` in [`Identity.sessions`](/reference/engine/tenant/sessions)); `person_token_id` is not set on these events.
...
A **fail-open** — re-validation could not be performed but the session was kept — is recorded as `idp_session_revalidation_failed` (with `success: true`, since it is not a security failure), `event_data: {apiKeyId}` and an `error_code`:
