...
By default, users with the global roles `super_admin` and `project_admin`, along with project-level `admin`, are authorized to issue invitations. However, you can extend this capability to other user roles by configuring [Tenant ACL permissions](/reference/engine/schema/acl#tenant-permissions).
...
By default, the invitation process auto-generates a password and sends it via email. However, it's recommended to set the invite method to `RESET_PASSWORD`. This way, a reset token is sent instead of a generated password. Ensure your [mail templates](/reference/engine/tenant/mail-templates) are appropriately configured to include the password setup link. Note that the default method will transition to `RESET_PASSWORD` in future updates.

### Customizing Email Templates

You can specify a preferred email template variant by setting the `mailVariant` option, as outlined in the [mail templates](/reference/engine/tenant/mail-templates) section.

### Audit

*(since 2.2)* Every `invite` / `unmanagedInvite` call is recorded in the [audit log](/reference/engine/tenant/audit-log) as `person_invite` with `event_data` carrying `{projectSlug, email, isNew, memberships}`.
