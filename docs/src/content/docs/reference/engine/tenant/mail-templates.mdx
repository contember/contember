...
Mail templates in Contember enable tailored communication for emails dispatched by the Tenant API. Via the `/tenant` path, users with the
appropriate built-in permission can create and manage templates. A [custom role](/reference/engine/tenant/custom-roles) can also receive separately
configured `mailTemplate:add`, `mailTemplate:remove`, and `mailTemplate:list` grants, limited to exact global/project scopes and mail types.
...
| `UNUSUAL_LOGIN` | A sign-in scores as unusual under [anomaly detection](/reference/engine/tenant/anomaly-detection). Informational notice; variables: `email`, `geoCountry`, `ipAddress`. | 2.2 |
