...
### Adding a Mail Template  
  Utilize the `addMailTemplate` mutation to introduce a new email template. Below is a comprehensive example, showcasing all possible fields:
   ...
   ```
  Input breakdown:
- `type`: The mail type — one of the values from the table above (`NEW_USER_INVITED`, `EXISTING_USER_INVITED`, `RESET_PASSWORD_REQUEST`, `PASSWORDLESS_SIGN_IN`, `FORCED_SIGN_OUT`, `EMAIL_VERIFICATION`, `EMAIL_CHANGE_VERIFY`, `EMAIL_CHANGE_NOTIFY`).
