...
*(since 2.2)* For a person linked to an identity provider with [`disableLocalAuthentication`](/reference/engine/tenant/idp#idp-only-authentication) the endpoint also returns `ok: true` and sends no mail — `signIn` would refuse the password anyway, so the mail would only be a link worth phishing.
