...
- *(since 2.2)* A person linked to an identity provider with [`disableLocalAuthentication`](/reference/engine/tenant/idp#idp-only-authentication) gets `IDP_REQUIRED` from both `initSignInPasswordless` and `signInPasswordless` — the latter even for a magic link issued before they were linked. On init it is masked as `PASSWORDLESS_DISABLED` unless both `login.revealUserExists` and `login.revealLoginMethod` are `true`.
...
Error codes: `PERSON_NOT_FOUND` (or `PASSWORDLESS_DISABLED` when `revealUserExists: false`), `INVALID_CAPTCHA` *(since 2.2)*, `RATE_LIMIT_EXCEEDED` *(since 2.2)*, `IDP_REQUIRED` *(since 2.2)*.
...
*(since 2.2)* Returns `IDP_REQUIRED` when the person is linked to an [IdP-only provider](/reference/engine/tenant/idp#idp-only-authentication), even for a magic link issued before they were linked.
...
The mutations are gated by the `PERSON_TOGGLE_PASSWORDLESS` permission, which is held by `PERSON` (any authenticated person) by default.

Three fields on `Person` describe the state, and a UI wants the last two:

| Field | Meaning |
|---|---|
| `passwordlessEnabled` | The person's own opt-in. `null` means "follow the policy". On its own it does not say whether signing in this way works. |
| `passwordlessAvailable` | Whether passwordless sign-in actually works for this person right now — the policy resolved against the opt-in, exactly as `signIn` resolves it. |
| `passwordlessSelfManaged` | Whether the opt-in decides anything at all. `false` under `always` / `never`, i.e. precisely when the toggle mutations answer `CANNOT_TOGGLE`. |

Branching on `passwordlessEnabled` alone is the common mistake: under `optOut` it is `null` for every account created since the policy was set (which means *enabled*), and under `always` / `never` it is ignored entirely. Query `me { person { passwordlessAvailable passwordlessSelfManaged } }` and offer the toggle only when `passwordlessSelfManaged` is true.
