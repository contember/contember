...
| `canManageConfiguration` | `system:configure` | `configure`, the policy / IdP / mail-template mutations, and reading `authPolicies` |
...
`idp:list` and `mailTemplate:list`, which have no introspection flag yet. With the built-in roles
this is invisible, because `SUPER_ADMIN` and `PROJECT_ADMIN` hold all four — it matters once a custom
role bundle grants them separately.
