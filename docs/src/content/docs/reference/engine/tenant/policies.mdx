...
| `subject.targetRoles` | person actions on another identity: `person.disable`, `person.changeProfile`, `person.changePassword`, `person.createSessionToken`, `person.forceSignOut`, `person.viewSessions` | the roles already held by the target |
...
An **array** tag substituted into a `${...}` placeholder is comma-joined into a
single string — there is no expansion into multiple actions, resources, or
condition values. With `tags: { teams: ["a", "b"] }`, both
`"project:${assignment.tags.teams}"` and a standalone `"${assignment.tags.teams}"`
bake to one value (`"project:a,b"` / `"a,b"`), never two. To scope a policy to
several actions/resources, list them explicitly rather than relying on tag
expansion.
...
- **Denylist on person actions.** `createSessionToken`, `disablePerson`,
  `changePersonProfile`, `changePersonPassword`, `forceSignOut`, and
  `viewSessions` are denied when `subject.targetRoles` contains `super_admin`
  or `project_creator` — a project admin cannot impersonate, disable,
  force-sign-out, or read the sessions of a higher-privileged identity.
