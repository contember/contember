...
| `lastIp`, `lastUserAgent` | Client info from a recent request. Written at most once per 60 s, so a change of IP or User-Agent shows up within a minute. |
...
```
   ...
   - expires_at              (60 s throttle)               api_key.disabled_at = now()
