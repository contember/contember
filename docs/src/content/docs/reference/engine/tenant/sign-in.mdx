...
All of them rely on the same per-IP rate limiting, captcha, and `revealUserExists` masking described in [anti-abuse](/reference/engine/tenant/anti-abuse). All of them are recorded in the [audit log](/reference/engine/tenant/audit-log).
