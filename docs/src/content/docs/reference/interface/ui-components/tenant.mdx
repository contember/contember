...
*(since 2.2)* These four views display configuration that is written with [`contember tenant:apply`](/reference/engine/tenant/declarative-config). There is no editing counterpart by design — each view says so, rather than offering an affordance it would have to refuse.
...
None of the three crashes the page. The read-only configuration views above throw in the same way as `authLog` and render the same kind of notice. See [permission introspection](/reference/engine/tenant/permissions) for asking up front what the caller may do, and [Tenant ACL permissions](/reference/engine/schema/acl#tenant-permissions) for granting these.
