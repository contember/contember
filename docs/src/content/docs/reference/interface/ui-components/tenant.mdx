...
| Export | What it does | From |
|---|---|---|
| `InviteForm` / `InviteFormFields` | Invite a new person by e-mail, with initial project roles | client-tenant / ui-lib-tenant |
...
Tenant configuration, auth policies, mail templates, and identity-provider administration are deliberately **code/IaC-driven** and have no corresponding UI component — see the [Tenant API reference](/reference/engine/tenant/overview) for those.
