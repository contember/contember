...
import { ListMembersMemberType, MemberIdentity, useListMembersQuery } from '../hooks'
				...
				{query.data.project.members.map(member => {
					return (
						<TableRow key={member.identity.id}>
							<TableCell><Identity identity={member.identity} /></TableCell>
							<TableCell>
							...
							<TableCell shrunk>
								<PageLinkButton
									size="small"
									to={createEditIdentityLink(member.identity.id)}
								>
									Edit roles
								</PageLinkButton>
							</TableCell>
