...
import { EntityRealm, EntityState, EntityStateStub, StateNode, StateType } from '../state'
...
export class EntityOperations {
	...
	public addEventListener(state: EntityState, type: EntityAccessor.EntityEventType, ...args: unknown[]) {
	...
	public batchUpdates(state: EntityState, performUpdates: EntityAccessor.BatchUpdatesHandler) {
		...
		this.eventManager.syncOperation(() => {
			this.batchUpdatesImplementation(state, () => {
				...
				for (const hasOneMarker of hasOneMarkers) {
					...
					const newEntityState = this.stateInitializer.initializeEntityState(new UnpersistedEntityKey(), {
						creationParameters: hasOneMarker.relation,
						environment: hasOneMarker.environment,
						initialEventListeners: hasOneMarker.relation,
						markersContainer: hasOneMarker.fields,
	...
	public onChildUpdate(state: EntityState, updatedState: StateNode) {
	...
	public addEntityRealm(targetState: EntityState | EntityStateStub, newRealm: EntityRealm) {
			...
			if (field instanceof FieldMarker) {
				...
				if (fieldState === undefined) {
					...
					for (const [childKey, childState] of fieldState.children) {
						this.stateInitializer.initializeEntityState(childState.id, {
	...
	private batchUpdatesImplementation(state: EntityState, performUpdates: EntityAccessor.BatchUpdatesHandler) {
	...
	private processEntityDeletion(state: EntityState, deletedChildState: EntityState) {
		...
		if (deletedChildState.id.existsOnServer) {
			if (state.plannedHasOneDeletions === undefined) {
				state.plannedHasOneDeletions = new Map()
			}
			for (const placeholderName of relevantPlaceholders) {
				state.plannedHasOneDeletions.set(placeholderName, deletedChildState)
			}
		}

		const realmsByPlaceholder = deletedChildState.realms.get(state)
		for (const placeholderName of relevantPlaceholders) {
			...
			state.children.set(
				placeholderName,
				this.stateInitializer.initializeEntityStateStub(
					new UnpersistedEntityKey(),
					new Map([[state, new Map([[placeholderName, realm]])]]),
				),
			)
		}
