...
import type { TenantGlobalApiKey, TenantProjectApiKey } from '../../../lib/tenant/clients/TenantApiKeyClient.js'
import { formatMemberships } from '../member/memberOptions.js'
...
export class TenantApiKeyListCommand extends Command<Args, Options> {
	...
	protected async execute(input: Input<Args, Options>, output: Output): Promise<void> {
		...
		if (project === undefined) {
			const apiKeys = await this.tenantClientProvider.apiKey().listGlobalApiKeys()
			output.table<TenantGlobalApiKey>(
				[
					{ field: 'id', name: 'ID' },
					{ field: 'identityId', name: 'Identity' },
					{ field: 'description', name: 'Description' },
					{ field: 'globalRoles', name: 'Global Roles', format: value => value === null ? '(not visible)' : value.map(humanText).join(', ') || '-' },
		...
		const apiKeys = await this.tenantClientProvider.apiKey().listProjectApiKeys(project)
		output.table<TenantProjectApiKey>(
			[
				{ field: 'id', name: 'ID' },
				{ field: 'identityId', name: 'Identity' },
				{ field: 'description', name: 'Description' },
