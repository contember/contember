...
export const FieldType = ({ field }: { field: Model.AnyField }) => {
	if (field.type === 'ManyHasMany') {
		return (
			<>
				<EntityLink entity={field.target} />[] <TargetField field={field} /> (m:n)
			</>
		)
	} else if (field.type === 'OneHasMany') {
		return (
			<>
				<EntityLink entity={field.target} />[] <TargetField field={field} /> (1:n)
			</>
		)
		...
		return (
			<>
				...
				<EntityLink entity={field.target} /> <TargetField field={field} /> (1:1)
			</>
		)
		...
		return (
			<>
				<NullableMark field={field} />
				<EntityLink entity={field.target} /> <TargetField field={field} /> (m:1)
			</>
		)
	} else {
		return (
