...
interface UpdateCustomRoleResult {
	updateCustomRole?: MutationResult
}
...
export class TenantClient {
	...
	public async updateCustomRole(slug: string, role: TenantCustomRoleConfig): Promise<void> {
		const result = role.description === null
			? await this.apiClient.execute<UpdateCustomRoleResult>(clearCustomRoleDescriptionMutation, {
				variables: { slug, grants: role.grants },
			})
			: await this.exec(updateCustomRoleFetcher, {
				slug,
