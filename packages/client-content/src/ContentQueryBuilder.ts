...
export class ContentQueryBuilder {
	...
	private createMutationOperation(
	...
	): ContentMutation<MutationResult> {
		const context = this.createContext(name)
		...
		return new ContentOperation('mutation', fieldName, args, selectionSet, it => {
			...
			return {
				...it,
				node: it.node
					? nodeSelection.transformFn(it.node, {
						rootValue: it.node,
					})
					: null,
			}
	...
	public transaction(
		...
		return new ContentOperation<TransactionResult<any>, 'mutation'>(
			'mutation',
			'transaction',
			transactionArgs,
			items,
			({ ok, errorMessage, errors, validation, ...data }) => {
				return {
					ok,
					errorMessage,
					errors,
