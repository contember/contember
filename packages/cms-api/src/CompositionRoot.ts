...
class CompositionRoot {
  composeServer(env: Env, projects: Array<Project>): Express {
    ...
    const masterContainer = new Container.Builder({})
      ...
      .addService('authMiddleware', ({tenantContainer}) => new AuthMiddlewareFactory(tenantContainer.get('apiKeyManager')).create())
      .addService('tenantMiddleware', ({tenantContainer}) => new TenantMiddlewareFactory(tenantContainer.get('apolloServer')).create())
      .addService('contentMiddlewareFactoryMiddleware', ({projectContainers}) => new ContentMiddlewareFactoryMiddlewareFactory(projectContainers).create())
      .addService('contentMiddleware', ({}) => new ContentMiddlewareFactory().create())

      .addService('express', ({authMiddleware, tenantMiddleware, contentMiddlewareFactoryMiddleware, contentMiddleware}) => {
        const app = express()
