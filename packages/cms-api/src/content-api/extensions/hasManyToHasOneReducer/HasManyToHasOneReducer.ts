...
class HasManyToHasOneReducer implements EntityFieldsProvider<HasManyToHasOneReducer.Meta> {
	...
	constructor(private readonly schema: Model.Schema, private readonly visitor: HasManyToHasOneRelationReducerFieldVisitor) {
	}

	getFields(entity: Model.Entity, fields: string[]): EntityFieldsProvider.FieldMap<HasManyToHasOneReducer.Meta> {
		return fields.reduce((result, field) => ({ ...result, ...acceptFieldVisitor(this.schema, entity, field, this.visitor) }), {})
	}
}
