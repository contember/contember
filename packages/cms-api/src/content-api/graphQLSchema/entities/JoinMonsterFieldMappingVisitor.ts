...
type JoinMonsterFieldMappingWithWhere = JoinMonsterFieldMapping<any, any> & {
	args?: { where: { type: GraphQLInputObjectType } }
}
export default class JoinMonsterFieldMappingVisitor
	implements
		Model.ColumnVisitor<JoinMonsterFieldMapping<any, any>>,
			...
			junction: {
				...
				sqlBatch: {
					...
					sqlJoin: (tableName, targetTable) => {
						return `${tableName}.${targetRelation.joiningTable.joiningColumn.columnName} = ${targetTable}.${
		...
		return {
			junction: {
				...
				sqlBatch: {
					...
					sqlJoin: (tableName, targetTable) => {
						return `${tableName}.${relation.joiningTable.inverseJoiningColumn.columnName} = ${targetTable}.${
							targetEntity.primaryColumn
						}`
					}
				}
	...
	public getHasManyMapping(relation: Model.Relation, targetEntity: Model.Entity) {
		return {
			...
			where: (tableAlias: string, args: { where: Input.Where }, context: any, sqlAstNode: any) => {
				...
				return buildWhere(this.schema, targetEntity, joinToAst(this.schema, createAlias)(sqlAstNode, targetEntity))(
					tableAlias,
					args.where || {}
				)
			}
