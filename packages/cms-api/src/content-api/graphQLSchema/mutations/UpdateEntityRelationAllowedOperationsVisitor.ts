...
export default class UpdateEntityRelationAllowedOperationsVisitor
	...
	public visitManyHasOne(entity: Model.Entity, relation: Model.ManyHasOneRelation, targetEntity: Model.Entity) {
		...
		return operations.filter(it => ![Input.UpdateRelationOperation.delete, Input.UpdateRelationOperation.disconnect].includes(it))
	}
	...
	private getAllowedOperations(targetEntity: Model.Entity, owningEntity: Model.Entity, owningRelation: Model.Relation): Input.UpdateRelationOperation[] {
		...
		const canUpdateOwningRelation = this.authorizator.isAllowed(Acl.Operation.update, owningEntity.name, owningRelation.name)
