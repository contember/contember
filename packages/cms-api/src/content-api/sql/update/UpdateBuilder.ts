...
export default class UpdateBuilder {
	...
	private async createUpdatePromise(blocker: PromiseLike<void>) {
		...
		qb.with('newData_', qb => {
			...
			const allColumns: string[] = Object.values(acceptEveryFieldVisitor(this.schema, this.entity, {
		...
		const columns: QueryBuilder.ColumnExpressionMap = resolvedData.reduce<QueryBuilder.ColumnExpressionMap>((result: object, item) => (
			{...result, [item.columnName]: expr => expr.select(['newData_', item.columnName])}
		), {})

		return await qb.updateFrom(this.entity.tableName, columns, qb => {
			qb.from('newData_')
			this.whereBuilder.build(qb, this.entity, new Path([], this.entity.tableName), {and: [this.uniqueWhere, this.oldWhere]})
