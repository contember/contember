...
import { Actions, ActionsPayload, Result } from '@contember/schema'
...
export class TriggerPayloadPersister {
	...
	public async persist(trigger: Actions.AnyTrigger, payloads: ActionsPayload.AnyEventPayload[]): Promise<Result.TriggeredAction[]> {
		const chunkSize = 100
		if (!this.schemaId) {
		...
		for (let i = 0; i < payloads.length; i += chunkSize) {
			...
			await InsertBuilder.create()
				...
				.values(chunk.map((it): EventRowToInsert => {
					...
					triggeredActions.push({ id, trigger: trigger.name, target: trigger.target })
					return {
						id,
