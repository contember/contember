...
export class PredicatesInjector {
	...
	public inject(
		...
		const expandedWhere = this.expandRelationAbsence(where, entity, ancestorPath ?? [], isQueryRoot)
		const restrictedWhere = this.injectToWhere(expandedWhere, entity, true, relationContext, false, ancestorPath ?? [], isQueryRoot)
		return this.createWhere(entity, undefined, restrictedWhere, relationContext, false, ancestorPath ?? [], isQueryRoot)
	}

	/**
	 * Is `where` exactly the "no related row" idiom `{ <primary>: { isNull: true } }`?
	 */
	private isPrimaryIsNullTest(where: Input.OptionalWhere, primary: string): boolean {
		const keys = Object.keys(where)
		if (keys.length !== 1 || keys[0] !== primary) {
			return false
		}
		const cond = where[primary] as Input.Condition | null | undefined
		return cond != null && (cond as Input.Condition).isNull === true && Object.keys(cond).length === 1
	}

	/**
	 * Rewrites the "no related row" idiom `{ rel: { <primary>: { isNull: true } } }` so that under ACL it
	 * means "no READABLE related row". A present-but-unreadable row is invisible to the role (reading the
	 * relation returns nothing), so it must look absent here too — otherwise `isNull` would (a) fail to
	 * match such a parent (leaking that a hidden row exists) and (b) disagree with `isNull:false`/`not(...)`,
	 * violating the law of excluded middle for the role.
	 *
	 * The fix replaces the raw test with `not (a readable related row exists)`:
	 *   { rel: { id: { isNull: true } } }  →  { not: { rel: { id: { isNull: false } } } }
	 * `{ id: { isNull: false } }` is the "a present row exists" probe; injectToWhere ANDs the relation's read
	 * predicate into it, so this lowers uniformly to `NOT EXISTS(<readable rel>)` for has-many / many-has-many
	 * and to the equivalent left-join form for to-one. It matches the parent iff the role can read no related
	 * row — covering the genuinely-absent row (primary IS NULL) and the present-but-unreadable row alike.
	 * Only applied when the target entity actually has a read predicate; public relations keep the plain test.
	 */
	private expandRelationAbsence(
		where: Input.OptionalWhere,
		entity: Model.Entity,
		ancestorPath: readonly Model.AnyRelationContext[],
		isQueryRoot?: boolean,
		...
		if (where.and) {
			result.and = where.and.filter((it): it is Input.Where => !!it).map(it => this.expandRelationAbsence(it, entity, ancestorPath, isQueryRoot))
		}
		if (where.or) {
			result.or = where.or.filter((it): it is Input.Where => !!it).map(it => this.expandRelationAbsence(it, entity, ancestorPath, isQueryRoot))
		}
		if (where.not) {
			result.not = this.expandRelationAbsence(where.not, entity, ancestorPath, isQueryRoot)
		}

		const orForms: Input.Where[] = []
		const fields = Object.keys(where).filter(it => !['and', 'or', 'not'].includes(it))
		for (const field of fields) {
			const value = acceptFieldVisitor<Input.OptionalWhere | null | undefined>(this.schema, entity, field, {
				visitColumn: () => where[field] as Input.OptionalWhere,
				visitRelation: context => {
					const sub = where[field] as Input.OptionalWhere | null
					if (sub === null) {
						return null
					}
					const hasReadPredicate = Object.keys(
						this.predicateFactory.create(context.targetEntity, Acl.Operation.read, undefined, context, isQueryRoot),
					).length > 0
					if (this.isPrimaryIsNullTest(sub, context.targetEntity.primary) && hasReadPredicate) {
						// "no readable related row" = not (a present readable row exists)
						const presentRow = { [field]: { [context.targetEntity.primary]: { isNull: false } } } satisfies Input.Where
						orForms.push({ not: presentRow })
						return undefined
					}
					return this.expandRelationAbsence(sub, context.targetEntity, [...ancestorPath, context], isQueryRoot)
				},
			})
			if (value !== undefined) {
				result[field] = value
		...
		const parts = [result, ...orForms].filter(it => Object.keys(it).length > 0)
		if (parts.length === 1) {
			return parts[0]
		}
		return { and: parts }
