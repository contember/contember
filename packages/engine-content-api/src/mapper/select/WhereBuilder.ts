...
export class WhereBuilder {
	...
	private hasRootIsNull(where: Input.Where, entity: Model.Entity): boolean {
		for (const key in where) {
			if (key === 'and' && where[key]?.some(it => this.hasRootIsNull(it, entity))) {
				return true
			}
			if (key === 'or' && where[key]?.some(it => this.hasRootIsNull(it, entity))) {
				return true
			}
			if (key === 'not' && where.not && this.hasRootIsNull(where.not, entity)) {
				return true
			}
