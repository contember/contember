...
import { SchemaBuilder, InputValidation as v, AllowAllPermissionFactory } from '@contember/schema-definition'
import 'jasmine'
...
describe('Insert mutation', () => {
	...
	it('insert author with id', async () => {
	...
	it('insert site with empty settings', async () => {
		await execute({
			schema: new SchemaBuilder()
				.entity('Site', entity =>
					entity
	...
	it('insert category and connect existing post (one has many)', async () => {
		await execute({
			schema: new SchemaBuilder()
				.entity('Post', e =>
					e
