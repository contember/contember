	...
	await execute({
		...
		executes: [
			...sqlTransaction([
				...
				{
					sql:
						'with "newData_" as (select ? :: uuid as "setting_id", "root_"."id", "root_"."name"  from "public"."site" as "root_"   where "root_"."id" = ?) update  "public"."site" set  "setting_id" =  "newData_"."setting_id"   from "newData_"   where "site"."id" = "newData_"."id"',
					parameters: [settingId, siteId],
					response: { rowCount: 1 },
				},
				{ sql: 'SET CONSTRAINTS ALL DEFERRED', parameters: [], response: {} },
				{
				...
				{
					sql:
						'delete from "public"."site_setting"   where "id" in (select "root_"."id"  from "public"."site_setting" as "root_"   where "root_"."id" = ?)  returning "id"',
					parameters: [settingId2],
					response: { rows: [{ id: settingId2 }] },
				},
				{ sql: 'SET CONSTRAINTS ALL IMMEDIATE', parameters: [], response: {} },
			]),
		],
		return: {
			data: {
