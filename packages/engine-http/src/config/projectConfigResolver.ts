...
export const createProjectConfigResolver =
	(env: Env, config: any, configProcessors: ConfigProcessor<any>[]): ProjectConfigResolver =>
...
const createProjectParametersResolver = (slug: string, env: Env, secrets: ProjectSecrets): ParametersResolver => parts => {
	if (parts[0] === 'project') {
		if (parts[1] === 'env') {
