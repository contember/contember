...
export type PgColumnSchema = { name: string, type: ColumnType, nullable: boolean }
...
export class PgSchemaBuilder {
	...
	private static buildPgColumnSchemaMap(schema: Schema, entity: Entity): PgColumnSchemaMap {
		...
		for (const field of Object.values(entity.fields)) {
			const column = acceptFieldVisitor(schema.model, entity, field, {
				...
				visitOneHasOneOwning: (entity, relation, targetEntity) => ({
					...
					type: targetEntity.fields[targetEntity.primary].type as ColumnType,
	...
	private static collectManyHasManyOwned(schema: Schema, entity: Entity): PgTableSchema[] {
		const joiningTables = []
				...
				visitManyHasManyOwning: (entity, relation, targetEntity) => ({
					...
					columns: {
						[relation.joiningTable.joiningColumn.columnName]: {
							...
							type: entity.fields[entity.primary].type as ColumnType,
							nullable: false,
						},
