...
describe('panel controller', () => {
	...
	test('treats dollar sequences in the configured mount path literally', () => {
		const controller = createController('/console/$&')
		const html = gunzipSync(controller.handle({ method: 'GET', path: undefined, acceptEncoding: 'gzip' }).body).toString('utf8')

		expect(html).toContain('<base href="/console/$&/">')
		expect(html).toContain('"basePath":"/console/$&/"')
	})
	...
	test('does not decompress a HEAD response body', () => {
		const response = get('assets/index-abc123.js', { method: 'HEAD', acceptEncoding: undefined })
...
describe('panel asset store', () => {
	...
	test('decodes an asset once', () => {
		...
		expect(store.getIdentity('assets/index-abc123.js')).toBe(store.getIdentity('assets/index-abc123.js'))
	})
})
