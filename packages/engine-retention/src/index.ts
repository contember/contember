...
export default class RetentionPlugin implements Plugin {
	...
	constructor(registry: ScheduledJobRegistry) {
		registry.register(new RetentionJob(() => this.config, () => this.metrics))
	}
	...
	getMasterContainerHook(): MasterContainerHook {
		return (builder: MasterContainerBuilder) =>
			...
			// config + metrics for the job even though the plugin registers no worker of its own.
			builder.setupService('applicationWorkers', (_applicationWorkers, { promRegistry, serverConfig }) => {
