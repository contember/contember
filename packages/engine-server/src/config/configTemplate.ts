export const configTemplate: any = {
	tenant: {
		db: {
			...
			queryTimeoutMs: '%?tenant.env.DB_QUERY_TIMEOUT_MS%',
			statementTimeoutMs: '%?tenant.env.DB_STATEMENT_TIMEOUT_MS%',
			connectionTimeoutMs: '%?tenant.env.DB_CONNECTION_TIMEOUT_MS%',
			pool: {
				maxConnections: '%?tenant.env.DB_POOL_MAX_CONNECTIONS%',
