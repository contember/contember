...
import { AuthMiddlewareState, ErrorResponseMiddlewareState } from '../common'
...
export const createProjectMemberMiddleware = (): KoaMiddleware<KoaState> => {
	const projectMember: KoaMiddleware<KoaState> = async (ctx, next) => {
		...
		const projectMemberships = await ctx.state.projectMemberManager.getProjectBySlugMemberships(project.slug, {
