...
const createSingleDbPoolMetricsCollector = (registry: prom.Registry) => {
	...
	const totalCount = new prom.Gauge({
		...
		help:
			'The total number of clients existing within the pool by contember_module (tenant or content; system is not used since it uses the same connection) and contember_project for contember_module=content.',
	...
	const idleCount = new prom.Gauge({
		...
		help:
			'The number of clients which are not checked out but are currently idle in the pool by contember_module (tenant or content; system is not used since it uses the same connection) and contember_project for contember_module=content.',
		labelNames: labelNames,
	})
	...
	const maxCount = new prom.Gauge({
		...
		help:
			'Maximum number of clients the pool should contain by contember_module (tenant or content; system is not used since it uses the same connection) and contember_project for contember_module=content.',
		labelNames: labelNames,
	})
...
const createSqlMetrics = (
	...
	const sqlDuration = new prom.Histogram({
		...
		help:
			'Executed SQL queries by contember_module (system, tenant, content or unknown) and contember_project (or "unknown" for unknown project)',
		registers: [registry],
