...
import { SameRowDependencyBuilder, TransactionDependencyBuilder } from '../../../../src/model/events/dependency'
...
const test = suite('Same row dependency builder')
...
test('basic deps', async () => {
	...
	const dependencyBuilder = new SameRowDependencyBuilder()
	...
	const events = [
		...
		new UpdateEvent(event3, now, unnamedIdentity, trxId1, [authorEntityId], 'author', {}),
	]
	...
	assert.is(event1deps.size, 3)
	assert.ok(event1deps.has(event2))
	assert.ok(event1deps.has(event3))
	const event2deps = deps.get(event2)!
	assert.is(event2deps.size, 3)
	assert.ok(event2deps.has(event1))
	assert.ok(event2deps.has(event3))

	const event3deps = deps.get(event3)!
	assert.is(event3deps.size, 3)
	assert.ok(event3deps.has(event1))
	assert.ok(event3deps.has(event2))
})
