...
- **IdP-only authentication** (`LocalAuthDisablingIdpsQuery`): with `options.disableLocalAuthentication` on a provider, a person linked to it loses every local authentication path — password sign-in and passwordless sign-in answer `IDP_REQUIRED`, and `createResetPasswordRequest` reports ok but sends no mail. The `person_identity_provider` link is the marker (not `person.idp_only`, which is written at auto-sign-up but enforced nowhere), so it also covers a person who predates the provider and was linked by e-mail matching. A **disabled** provider does not count: turning the provider off restores local authentication for everyone linked to it, which is the break-glass for an IdP outage. Permanent API keys are unaffected — they hold their own identity with no person. `disconnectMyIdentityProvider` refuses such a link with `IDP_REQUIRED`, even while the provider is disabled — the link is what enforces the policy, so the person it binds must not be able to remove it
...
- Also note `Identity.sessions: [SessionInfo!]!` (own sessions always visible; others via `person:viewSessions`) and `Query.authLog` (`system:viewAuthLog`, PROJECT_ADMIN and SUPER_ADMIN by default).
...
- **`PROJECT_ADMIN`** is reached two ways: (a) **per-project**, granted dynamically inside a project scope (`ProjectScope.getIdentityAccess`) when the calling identity's membership in that project includes the project `admin` role — `PROJECT_MEMBER` + `PROJECT_ADMIN` are then unioned into the access node *for that project only*; or (b) as a **global** tenant role held in `identity.roles`, in which case `ProjectMemberManager.getImplicitProjectMemberships` grants implicit project `admin` membership on **all** projects (same as global `SUPER_ADMIN`). Its parameterized grants use dedicated verifier functions: valid custom roles are assignable, while `super_admin` and `project_creator` remain protected.
- `LOGIN` / `PERSON` cover the unauthenticated sign-in actions and the self-service actions of a signed-in person, respectively.
- **Custom roles** (`custom_role` table): runtime-defined global roles carrying explicit JSONB grants (`createCustomRole` / `updateCustomRole` / `deleteCustomRole`, gated by `customRole:manage`; listing and the grant-definition catalog via `customRoles` / `customRolePermissions`, gated by `customRole:view`). Assignment reuses `identity.roles`. `CustomRolePermissions.ts` is an explicit fail-closed registry limited to the PROJECT_ADMIN action surface; configured actions compile into the existing `Permissions.allow(..., verifier)` path. Strict action-specific JSON config uses `@contember/typesafe`; role constraints are an exhaustive `allowed` list (every role the target holds must appear in it), and `NON_DELEGABLE_TENANT_ROLES` in `Roles.ts` always protects `super_admin` / `project_creator` — for the static PROJECT_ADMIN verifiers and custom-role grants alike. Project creation and project/membership actions are absent; mail-template grants are the explicit project-aware exception and use exact project/type filters. `CustomRoleAuthorizator` wraps the static authorizator and **does not pass the `AuthorizationScope` down** — `Permissions.isAllowed` has no scope parameter, so any grant that could answer a project-scoped check would answer it for *every* project (unlike the equivalent static PROJECT_ADMIN grant, which `ProjectScope.getIdentityAccess` bounds). The cache therefore compiles two maps from the one query: the full one for `AuthorizationScope.Global` checks, and a restricted one for scoped checks holding only `PROJECT_BOUNDED_KINDS` — currently just `MAIL_TEMPLATE_SCOPE`, whose config carries its own exact project filter. A new configuration kind defaults to global-only, so it silently does nothing in a scoped context rather than escaping its bound. Definitions are loaded once per request — eagerly at context creation when the identity carries a non-builtin role, so no permission check has to query while holding a transaction open — and that cache is shared with derived identity contexts. Delete removes the definition and all assignments transactionally, freeing the slug; the audit entry is the trace. Definition changes audit the canonical before/after grants.

Notable permission actions: `person:list` (`PERSON_LIST`, tenant-wide person listing), `apiKey:list` (`API_KEY_LIST`, global permanent keys), `project:viewSecrets` (`PROJECT_VIEW_SECRETS`, granted to PROJECT_ADMIN + SUPER_ADMIN), `project:viewMembers` (`PROJECT_VIEW_MEMBER`, also gates `project.apiKeys`), `person:viewSessions`, `system:viewAuthLog`, `system:configure`, `customRole:manage` / `customRole:view` (PROJECT_ADMIN may view definitions; only SUPER_ADMIN may manage them by default).
