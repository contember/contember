...
- **Custom roles** (`custom_role` table): runtime-defined global roles carrying a bundle of permission actions (`createCustomRole` / `updateCustomRole` / `deleteCustomRole`, gated by `customRole:manage`; listing via `customRoles` / `customRolePermissions`, gated by `customRole:view`). Assignment reuses `identity.roles` (`addGlobalIdentityRoles`). Evaluation: `PermissionContextFactory` wraps the static `PermissionEvaluator` in a per-request `CustomRoleAccessEvaluator` that lazily loads non-builtin roles from the DB (at most one query per request, only when the static check denies). The grantable vocabulary (`CustomRolePermissions.ts`) is derived from `PermissionActions` minus escalation vectors (`identity:addGlobalRoles`, `identity:removeGlobalRoles`, `apiKey:createGlobal`, `person:createSessionToken`, `customRole:manage`); `roles`-parameterized grants always carry the forbidden-target-roles guard (no acting on `super_admin` / `project_creator`). Definition changes audit `custom_role_change`.
