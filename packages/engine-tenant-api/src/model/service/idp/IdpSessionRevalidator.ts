...
import { ApiKeyRow } from '../../queries/apiKey/ApiKeyQuery'
import { IdpSessionByApiKeyQuery, IdpSessionRow } from '../../queries/idp/IdpSessionByApiKeyQuery'
import { IDPHandlerRegistry } from './IDPHandlerRegistry'
import { ClaimIdpRevalidationCommand } from '../../commands/idp/ClaimIdpRevalidationCommand'
import { UpdateIdpSessionCommand } from '../../commands/idp/UpdateIdpSessionCommand'
...
export class IdpSessionRevalidator {
	...
	async revalidate(dbContext: DatabaseContext, readDbContext: DatabaseContext, apiKeyRow: ApiKeyRow): Promise<RevalidationOutcome> {
		...
		if (row.providerDisabledAt) {
			await this.revoke(dbContext, apiKeyRow, row, 'idp_disabled')
			return 'revoked'
		}
		...
		if (decision.blocking) {
			return await this.runRevalidation(dbContext, handler, row, apiKeyRow)
		}
	...
	private async runRevalidation(
		...
		if (result.status === 'revoked') {
			await this.revoke(dbContext, apiKeyRow, row, result.reason)
			return 'revoked'
		}
