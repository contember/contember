...
export class CustomRoleMutationResolver implements Pick<MutationResolvers, 'createCustomRole' | 'updateCustomRole' | 'deleteCustomRole'> {
	...
	async createCustomRole(parent: any, args: MutationCreateCustomRoleArgs, context: TenantResolverContext): Promise<CreateCustomRoleResponse> {
		...
		// the conflict is caught out here: a unique violation aborts the transaction, so
		// mapping it inside would return normally and then fail on COMMIT
		let result
		try {
			result = await context.db.transaction(async db => {
			...
			if (CustomRoleManager.isSlugConflict(error)) {
				return createErrorResponse('SLUG_ALREADY_EXISTS', `Custom role ${args.slug} already exists`)
			}
			throw error
		}
		if (!result.ok) {
