...
import { getGrantablePermissions, isConfigurationRequired } from '../../model/authorization/CustomRolePermissions.js'
	...
	async customRolePermissions(parent: unknown, args: unknown, context: TenantResolverContext): Promise<readonly CustomRolePermissionDefinition[]> {
		...
		return [...getGrantablePermissions().values()]
			...
			.map(definition => ({
				...
				configurationRequired: isConfigurationRequired(definition),
