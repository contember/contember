...
test('enablePerson checks permission against the target roles', async () => {
	const personId = testUuid(1)
	const identityId = testUuid(2)
	...
	await executeTenantTest({
		...
		executes: [
			getPersonByIdSql({
				personId,
				response: { personId, identityId, password: '123', roles: targetRoles, email: 'jane@doe.com', disabledAt: new Date('2019-09-04 12:00') },
			}),
		],
		return: {
			data: { enablePerson: null },
			errors: [{ message: 'You are not allowed to enable person account' }],
		},
	})
})
