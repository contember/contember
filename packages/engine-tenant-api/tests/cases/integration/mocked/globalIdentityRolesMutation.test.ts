...
test('removes a global identity role', async () => {
	...
	await executeTenantTest({
		...
		executes: [
			// a super_admin target short-circuits the membership-presence lookup
			selectIdentitySql({ identityId, roles: [role] }),
...
test('removes a global identity role from an unprivileged target', async () => {
	const identityId = testUuid(6)
	const personId = testUuid(7)
	const role = 'entrypoint_deployer'
	await executeTenantTest({
		query: GQL`mutation {
			removeGlobalIdentityRoles(identityId: "${identityId}", roles: ["${role}"]) {
				ok
