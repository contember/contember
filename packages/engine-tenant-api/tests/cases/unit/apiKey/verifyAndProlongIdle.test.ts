...
// The expiry/idle/max gates are now computed on the DB clock (is_expired /
// is_max_expired / is_idle_expired), so each case drives the gate via those booleans
// on the canned row. The dates below are kept for realism and for the prolong throttle.
...
test('A19: idle-expired key (last_used_at older than idle_timeout) is rejected and disabled', async () => {
	const row = defaultRow({
		...
		is_idle_expired: true, // the DB-clock gate
	})
...
test('A19: absolute cap (now >= max_expires_at) rejects and disables', async () => {
	const row = defaultRow({
		...
		is_max_expired: true, // the DB-clock gate
	})
