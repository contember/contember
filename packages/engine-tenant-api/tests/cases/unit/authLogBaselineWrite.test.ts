...
import { AuthLogService, DatabaseContext, LoginRiskAnalyzer, TenantResolverContextFactory, UNPERSISTED_ROOT_IDENTITY_ID } from '../../../src/index.js'
...
test('an unpersisted root actor is audited without a dangling identity foreign key', async () => {
	await runWithQuery({
		...
		parameters: [
			...
			null,
			(value: unknown) => {
				expect(value).toEqual({ invokedByUnpersistedIdentityId: UNPERSISTED_ROOT_IDENTITY_ID })
				return true
			},
		],
		response: { rowCount: 1 },
