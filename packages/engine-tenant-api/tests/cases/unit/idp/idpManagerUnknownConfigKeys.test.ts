...
import { IDPHandlerRegistry, IDPManager } from '../../../../src/model/service/idp'
import { OIDCProvider } from '../../../../src/model/service/idp/providers/OIDCProvider'
...
const OPTIONS = { autoSignUp: false, exclusive: false, initReturnsConfig: false }

const makeManager = () => {
	const registry = new IDPHandlerRegistry()
	registry.registerHandler('oidc', new OIDCProvider())
	return new IDPManager(registry)
}
...
describe('IDPManager — unknown configuration keys', () => {
	...
	test('reports every unknown key at once, so several typos take one round trip', async () => {
		...
		const response = await makeManager().addIDP(db, {
			slug: 'sso',
			type: 'oidc',
			configuration: { ...OIDC_BASE, fetchUserinfo: true, idTokenSignedResponseAlgo: 'RS256' },
			options: OPTIONS,
		})
		...
		expect(message).toContain('idTokenSignedResponseAlgo')
	})
