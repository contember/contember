...
describe('RedirectOnPersist static render', () => {
	it('exposes a static render', () => {
		expect(typeof (RedirectOnPersist as StaticRenderProvider).staticRender).toBe('function')
	})

	it('registers fields referenced by a binding routing target', () => {
		const env = Environment.create()
		const rendered = (RedirectOnPersist as StaticRenderProvider).staticRender!(
