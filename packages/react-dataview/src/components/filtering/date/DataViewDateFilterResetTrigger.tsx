 ...
 *
 * ### Props
 * - **`name`**: The name of the filter. If not provided, it will be inferred from the context.
 * - **`type`**: Specifies which part of the date range to reset:
 *   - `'start'`: Resets the start date.
 *   - `'end'`: Resets the end date.
 *   - If omitted, both start and end dates will be reset.
 * - **`children`**: The button element.
 *
 * ### Behavior
 * - The trigger is rendered only if there is a start or end date to reset.
 * - Clicking the trigger resets the specified part(s) of the date range.
 *
 * ### Example
 * ```tsx
