	...
	tenant: {
		...
		login: {
			...
			errorMessages: {
				...
				'IDP_REQUIRED': 'Please sign in with your identity provider',
		...
		configView: {
			...
			disabled: 'Disabled',
			notSet: 'Not set',
			/** An explicitly empty list — distinct from `notSet`, which is a value that was never configured. */
			emptyList: 'None',
			signup: 'Sign-up',
			...
			limit: 'Limit',
			window: 'Window',
			panelAccess: 'Management panel access',
		},
		authPolicyList: {
			scope: 'Scope',
			project: 'Project',
		...
		passwordlessToggle: {
			...
			enforcedEnabled: 'Passwordless sign-in is ENABLED for everyone by a tenant-wide policy, so it cannot be turned off here.',
			enforcedDisabled: 'Passwordless sign-in is turned off by a tenant-wide policy, so it cannot be enabled here.',
			toggleFailed:
