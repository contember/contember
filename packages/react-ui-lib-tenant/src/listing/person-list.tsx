...
export const PersonList = (props: { controller?: { current?: MemberListController }; roles?: RolesConfig }) => (
	<MemberList
		...
		tableHeaders={[dict.tenant.personList.email, dict.tenant.personList.name, dict.tenant.personsList.mfa]}
		tableColumns={it => (
			<>
				...
				<TableCell key="mfa">
					<MfaBadges otpEnabled={it.identity.person?.otpEnabled} emailOtpEnabled={it.identity.person?.emailOtpEnabled} />
				</TableCell>
			</>
		)}
	/>
