	...
	return (
		<>
			<div className="relative group" contentEditable={false}>
				<div className="bg-gray-500 h-0.5 rounded-sm absolute top-1/2 left-0 right-0 w-full opacity-0 transition-opacity delay-100 group-hover:opacity-100">
				</div>
				<div
					className="absolute -top-2 left-0 w-full h-4 flex justify-center items-center opacity-0 transition-opacity delay-100 duration-0 group-hover:opacity-100 cursor-pointer"
					onClick={onClick}
				>
...
export const BlockElement = memo(
	function BlockElement({ element, children, attributes, domElement = 'div', withBoundaries = false, className }: BlockElementProps) {
