...
const SIDEBAR_WIDTH_LARGE = '30rem'
...
const SidebarProvider = forwardRef<HTMLDivElement, SidebarProviderProps>(
	({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, children }, ref) => {
...
interface SidebarLayoutProps extends ComponentProps<'div'> {
	className?: string
	...
	(
		...
		if (collapsible === 'none') {
			return (
				<div
					className={cn(
						'flex h-full w-[--sidebar-width] 2xl:w-[--sidebar-width-large] flex-col bg-sidebar text-sidebar-foreground',
						className,
					)}
					ref={ref}
					{...props}
				...
				<div
					className={cn(
						...
						variant === 'floating' || variant === 'inset'
							...
							: 'group-data-[collapsible=icon]:w-[--sidebar-width-icon] 2xl:group-data-[collapsible=icon]:w-[--sidebar-width-icon-large]',
					)}
				/>
				<div
					className={cn(
						'duration-200 fixed inset-y-0 z-10 hidden h-svh w-[--sidebar-width] 2xl:w-[--sidebar-width-large] transition-[left,right,width] ease-linear md:flex',
						side === 'left'
						...
						variant === 'floating' || variant === 'inset'
							? 'p-2 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4)_+2px)] 2xl:group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon-large)_+_theme(spacing.4)_+2px)]'
							: 'group-data-[collapsible=icon]:w-[--sidebar-width-icon] group-data-[side=left]:border-r group-data-[side=right]:border-l 2xl:group-data-[collapsible=icon]:w-[--sidebar-width-icon-large]',
						className,
					)}
					{...props}
				>
...
>(({ className, ...props }, ref) => {
	return (
		<div
			ref={ref}
			data-sidebar="header"
			className={cn('flex flex-col gap-2 p-2 2xl:p-3', className)}
			{...props}
		/>
	)
})
...
>(({ className, ...props }, ref) => {
	return (
		<div
			ref={ref}
			data-sidebar="group"
			className={cn('relative flex w-full min-w-0 flex-col p-2 2xl:p-4', className)}
			{...props}
		/>
	)
})
