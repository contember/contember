...
export const trigger =
	<T>(definition: BasicTriggerDefinition): DecoratorFunction<T> => (entity: EntityConstructor<T>, context?: ClassDecoratorContext) => {
		triggersStore.update(entity, it => [...it, { type: 'basic', definition }], context)
	}

export const watch =
	<T>(definition: WatchTriggerDefinition): DecoratorFunction<T> => (entity: EntityConstructor<T>, context?: ClassDecoratorContext) => {
