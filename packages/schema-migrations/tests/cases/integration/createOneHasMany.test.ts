...
describe('create one has many relation (post with locales)', () =>
	testMigrations({
		original: { model: new SchemaBuilder().entity('Post', e => e).buildSchema() },
		updated: {
			model: new SchemaBuilder()
				.entity('Post', e => e.oneHasMany('locales', r => r.target('PostLocale').ownerNotNull().ownedBy('post').orderBy('title')))
				.entity('PostLocale', e =>
					e
						.unique(['post', 'locale'])
