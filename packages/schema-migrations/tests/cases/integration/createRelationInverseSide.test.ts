...
describe('create inverse side relation (post with locales)', () =>
	testMigrations({
		original: {
...
describe('create inverse side relation together with changing onDelete behaviour', () =>
	testMigrations({
		original: {
			model: new SchemaBuilder()
				.entity('Post', e => e)
		...
		updated: {
			model: new SchemaBuilder()
				.entity('Post', e => e)
				.entity('PostLocale', e => e.manyHasOne('post', r => r.target('Post').inversedBy('locales').onDelete(Model.OnDelete.cascade)))
				.buildSchema(),
		},
		diff: [
			{
		...
		sql:
			SQL`ALTER TABLE "post_locale" DROP CONSTRAINT "fk_post_locale_post_id_post_id"; ALTER TABLE "post_locale" ADD FOREIGN KEY ("post_id") REFERENCES "post"("id") ON DELETE CASCADE DEFERRABLE INITIALLY IMMEDIATE;`,
