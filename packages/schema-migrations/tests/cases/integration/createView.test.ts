...
describe('create a relation and a view', () =>
	testMigrations({
		original: createSchema(ViewAddRelationOriginalSchema),
		updated: createSchema(ViewAddRelationUpdateSchema),
		diff: [
			{
				...
				owningSide: {
					...
					type: 'ManyHasOne',
					target: 'Category',
					joiningColumn: { columnName: 'category_id', onDelete: 'restrict' },
				},
			},
			{
				modification: 'createView',
				entity: {
					...
					indexes: [],
					fields: {
						id: { name: 'id', columnName: 'id', nullable: false, type: 'Uuid', columnType: 'uuid' },
						article: {
							name: 'article',
							inversedBy: 'stats',
							nullable: true,
							type: 'OneHasOne',
							target: 'Article',
							joiningColumn: { columnName: 'article_id', onDelete: 'restrict' },
						},
