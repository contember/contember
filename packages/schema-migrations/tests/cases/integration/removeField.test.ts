...
describe('remove relation (one has many)', () =>
	testMigrations({
		original: {
			model: new SchemaBuilder()
		...
		updated: {
			model: new SchemaBuilder()
				.entity('Post', e => e)
				.entity('PostLocale', e => e.column('title', c => c.type(Model.ColumnType.String)).column('locale', c => c.type(Model.ColumnType.String)))
				.buildSchema(),
		},
		diff: [
...
describe('remove relation (many has many)', () =>
	testMigrations({
		original: {
			model: new SchemaBuilder()
				.entity('Post', e => e.column('title', c => c.type(Model.ColumnType.String)).manyHasMany('categories', r => r.target('Category')))
...
describe('remove relation (one has one)', () =>
	testMigrations({
		original: {
			model: new SchemaBuilder()
				.entity('Site', entity =>
...
describe('remove relation inverse side', () =>
	testMigrations({
		original: {
			model: new SchemaBuilder()
				.entity('Site', entity =>
		...
		updated: {
			model: new SchemaBuilder()
				.entity('Site', entity => entity.column('name', c => c.type(Model.ColumnType.String)).oneHasOne('setting', r => r.target('SiteSetting')))
				.entity('SiteSetting', e => e.column('url', c => c.type(Model.ColumnType.String)))
				.buildSchema(),
		},
		diff: [
...
describe('test drop index / unique when removing a field', () =>
	testMigrations({
