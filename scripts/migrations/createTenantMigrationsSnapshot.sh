...
docker-compose exec -T -e PGUSER=contember -e PGPASSWORD=contember -e PGDATABASE=tenant_migrations -e PGHOST=postgres -e CONTEMBER_MIGRATIONS_NO_SNAPSHOT=1 engine \
	bash -c "bun ./packages/engine-tenant-api/src/bin/runMigrations.ts"
